...
//    private $serverUrl = 'https://api-rce-rcxtest.rongcloud.net/';
    ...
    public function Request($action, $params,$contentType='urlencoded',$module = 'im',$httpMethod='POST') {
        if ($this->isSwCoroutine()){
            $ret = $this->swRequest($action, $params, $contentType, $module, $httpMethod);
        }else{
            $ret = $this->curlRequest($action, $params, $contentType, $module, $httpMethod);
        }
        return $ret;
    }


    public function swRequest($action, $params,$contentType='urlencoded',$module = 'im',$httpMethod='POST'){
        switch ($module){
            case 'im':
        ...
        $swHeaders = [];
        foreach ($this->createHttpHeader() as $header){
        ...
        if ($httpMethod=='POST' && $contentType=='urlencoded') {
            $swHeaders['Content-Type'] = 'application/x-www-form-urlencoded';
        ...
        $params = $contentType=='json' ? json_encode($params) : $params;
        if ($httpMethod == 'POST'){
            $ret = $this->swPost($action, $params, $swHeaders);
        }elseif ($httpMethod == 'GET'){
            $ret = $this->swGet($action, $swHeaders);
        }

        return json_decode($ret, true);
    }

    public function swGet(string $url, array $headers = [], array $cookies = []){
        $urlInfo = parse_url($url);
        $port = $urlInfo['scheme'] === 'https' ? 443 : 80;
        $ssl = $port === 443 ? true : false;
        $client = new \Swoole\Coroutine\Http\Client($urlInfo['host'], $port, $ssl);
        if ($headers){
        ...
        $shortUrl = (isset($urlInfo['path']) ? $urlInfo['path'] : '')
            . (isset($urlInfo['query']) ? '?'.$urlInfo['query'] : '');
        $client->set(['timeout' => RongCloud::$timeout]);
        $client->get($shortUrl);
        $client->close();
        return $client->getBody();
    }

    public function swPost(string $url, $data = [], array $headers = [], array $cookies = []){
        $urlInfo = parse_url($url);
        $port = !empty($urlInfo['port']) ? $urlInfo['port'] : ($urlInfo['scheme'] === 'https' ? 443 : 80);
        $ssl = $port === 443 ? true : false;
        $client = new \Swoole\Coroutine\Http\Client($urlInfo['host'], $port, $ssl);
        if ($headers){
            $client->setHeaders($headers);
    ...
    public function curlRequest($action, $params,$contentType='urlencoded',$module = 'im',$httpMethod='POST'){
        switch ($module){
            case 'im':
                $action = $this->serverUrl.$action;
                break;
        ...
        curl_setopt($ch, CURLOPT_TIMEOUT, RongCloud::$timeout);
